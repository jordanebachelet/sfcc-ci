...
module.exports.cli = {
    realm : {
         ...
         */
        list : function(realm, topic, asJson, sortBy, callback) {
            // get details of a single realm if realm id was passed
            if ( typeof(realm) !== 'undefined' && realm !== null ) {
                getRealm(realm, topic, function (err, realmResult) {
                    if (typeof callback !== 'undefined') {
                        callback(err, realmResult);
                        return;
                    }
                    ...
                    if (asJson) {
                        console.json(realmResult);
                        return;
                    }
                    console.prettyPrint(realmResult);
                });
                return;
            }
            // get all realms
         ...
         * @param {Function} callback callback function to call when called through the Javascript API
         */
        update : function(realm, maxSandboxTTL, defaultSandboxTTL, asJson, callback) {
     ...
     * @param {Function} callback callback function to call when called through the Javascript API
     */
    list : function(showDeleted, asJson, sortBy, callback) {
     ...
     * @param {Function} callback callback function to call when called through the Javascript API
     */
    create : function(realm, alias, ttl, ocapiSettings, webdavPermissions, asJson, sync, setAsDefault, callback) {
    ...
    get : function(spec, asJson, hostOnly, openBrowser, topic, callback) {
        ...
        if ( !( spec['id'] || ( spec['realm'] && spec['instance'] ) ) ) {
            if (typeof callback !== 'undefined') {
                callback('Provide either an id or a realm and an instance of the sandbox.', undefined);
                return;
            }
            console.error('Provide either an id or a realm and an instance of the sandbox.');
            return;
    ...
    delete : function(spec, callback) {
        ...
        lookupSandbox(spec, function(err, foundSandbox) {
            if (err) {
                ...
                if (spec['realm'] && spec['instance']) {
                    if (typeof callback !== 'undefined') {
                        callback('Cannot find sandbox with realm %s and instance %s.', spec['realm'],
                            spec['instance'], undefined);
                        return;
                    }
                    ...
                    if (typeof callback !== 'undefined') {
                        callback('Cannot find sandbox with id %s.', spec['id'], undefined);
                        return;
                    }
                ...
                deleteSandbox(foundSandbox.id, function(err) {
                    ...
                    if (typeof callback !== 'undefined') {
                        callback(undefined, foundSandbox);
                        return;
                    }
     ...
     * @param {Function} callback callback function to call when called through the Javascript API
     */
    restart : function(spec, callback) {
        // id not provided, using other properties to lookup sandbox
        if (!( spec['id'] || ( spec['realm'] && spec['instance']))) {
            if (typeof callback !== 'undefined') {
                callback('Provide either an id or a realm and an instance of the sandbox.', undefined);
     ...
     * @param {Function} callback callback function to call when called through the Javascript API
     */
    update : function(spec, ttl, asJson, callback) {
        // id not provided, using other properties to lookup sandbox
        if ( !( spec['id'] || ( spec['realm'] && spec['instance'] ) ) ) {
            if (typeof callback !== 'undefined') {
                callback('Provide either an id or a realm and an instance of the sandbox.', undefined);
        ...
        lookupSandbox(spec, function(err, foundSandbox) {
            if (err) {
                ...
                updateSandbox(foundSandbox.id, ttl, function(err, updatedSandbox) {
                    ...
                    if (typeof callback !== 'undefined') {
                        callback(undefined, updatedSandbox);
                        return;
                    }
...
module.exports.api = {
    realm : {
         ...
         * @param {String} sortBy optional field to sort the list by
         *
         * @return {Promise} New promise that wraps the logic
         */
        list : function(realm, topic, sortBy) {
            return new Promise((reject, resolve) => {
                module.exports.cli.realm.list(realm, topic, false, sortBy, (err, result) => {
                    if (err) {
                        reject(err);
                        return;
                    }
         ...
         * @return {Promise} New promise that wraps the logic
         */
        update : function(realm, maxSandboxTTL, defaultSandboxTTL) {
            return new Promise((reject, resolve) => {
                module.exports.cli.realm.udpdate(realm, maxSandboxTTL, defaultSandboxTTL, false, (err, result) => {
                    if (err) {
                        reject(err);
                        return;
                    }
     ...
     * @param {String} sortBy optional field to sort the list of sandboxes by
     */
    list : function(showDeleted, sortBy) {
        return new Promise((reject, resolve) => {
            module.exports.cli.list(showDeleted, false, sortBy, (err, result) => {
                if (err) {
                    reject(err);
                    return;
                }
     ...
     * @param {Boolean} setAsDefault optional flag to set as new default instance, false by default
     */
    create : function(realm, alias, ttl, ocapiSettings, webdavPermissions, sync, setAsDefault) {
        return new Promise((reject, resolve) => {
            module.exports.cli.create(
                realm,
                alias,
                ttl,
                ocapiSettings,
     ...
     * @param {String} topic topic to retrieve details for
     */
    get : function(spec, hostOnly, topic) {
        return new Promise((reject, resolve) => {
            module.exports.cli.get(spec, false, hostOnly, false, topic, (err, result) => {
                if (err) {
                    reject(err);
                    return;
                }
    ...
    delete : function(spec) {
        return new Promise((reject, resolve) => {
            module.exports.cli.delete(spec, (err, result) => {
                if (err) {
                    reject(err);
                    return;
                }
    ...
    start : function(spec) {
        return new Promise((reject, resolve) => {
            module.exports.cli.start(spec, (err, result) => {
                if (err) {
                    reject(err);
                    return;
                }
    ...
    stop : function(spec) {
        return new Promise((reject, resolve) => {
            module.exports.cli.stop(spec, (err, result) => {
                if (err) {
                    reject(err);
                    return;
                }
    ...
    restart : function(spec) {
        return new Promise((reject, resolve) => {
            module.exports.cli.restart(spec, (err, result) => {
                if (err) {
                    reject(err);
                    return;
                }
    ...
    reset : function(spec) {
        return new Promise((reject, resolve) => {
            module.exports.cli.reset(spec, (err, result) => {
                if (err) {
                    reject(err);
                    return;
                }
     ...
     * @param {Number} ttl number of hours, the sandbox TTL will be prolonged
     */
    update : function(spec, ttl) {
        return new Promise((reject, resolve) => {
            module.exports.cli.update(spec, ttl, (err, result) => {
                if (err) {
                    reject(err);
                    return;
                }
